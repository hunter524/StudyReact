...
import {
  HighComponent,
  HighComponentSecond,
} from "./react_feature/hoc/HOCComponent";
...
import ParentLifeCycleComponent from "./react_feature/lifecycle/ParentLifeCycleComponent";
import getNumber from "./flow/flow";
  ...
  render() {
    return (
      <div>
        ...
        <a
          href="https://www.baidu.com"
          onClick={this.handleClickPreventWithParam.bind(this, this.props.name)}
        >
          prevent and invoke with param
        </a>
      </div>
    );
  }
    ...
    return (
      <div>
        {this.props.unReadMessage.length > 0 && (
          <h2>You have {this.props.unReadMessage.length} message!</h2>
        )}
        {this.props.unReadMessage.length >= 3 ? (
          <h2>More than Three message!</h2>
        ) : (
  ...
  render() {
    return (
      <div>
        ...
        <ul>
          {this.props.lists.map((v) => (
            <ListItemWrong key={v.toString()} value={v} />
          ))}
        </ul>
      </div>
    );
    ...
    return (
      <div>
        ...
        <MouseMove
          render={(state) => {
            return (
              <a style={{ position: "absolute", left: state.x, top: state.y }}>
                Cat
              </a>
            );
          }}
...
// 验证 flow.js 语法可以在 create-react-app 中直接使用无需配置
